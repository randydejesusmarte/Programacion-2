...
namespace Programacion_II
{
    public partial class Retiro : Form
    {
        ...
        private void Bt_gua_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        conectar.Abrir();
                        SqlCommand guardar = new SqlCommand("insert into Retiro (Num_cue, Nom_ben, Ced_cli, Tip_cue, Mon_ret, Mone) values ('" + Mt_num.Text + "','" + Tx_nom.Text + "','" + Mt_ced.Text + "','" + Cb_tip.SelectedItem.ToString() + "','" + Mt_mon.Text + "','" + Cb_tip.SelectedItem.ToString() + "')", conectar.SqlConnection);
        ...
        private void Bt_mod_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                if (formvalid())
                {
                    ...
                    {
                        conectar.Abrir();
                        SqlCommand command = new SqlCommand("Update retiro set Num_cue = '" + Mt_num.Text + "', Nom_ben = '" + Tx_nom.Text + "', Ced_cli = '" + Mt_ced.Text + "', Tip_cue = '" + Cb_tip.Text + "', Mon_ret = '" + Mt_mon.Text + "', Moneda = '" + Cb_mon.Text + "' where Id = '" + txt_id.Text + "'", conectar.SqlConnection);
        ...
        private void Bt_can_Click(object sender, EventArgs e)
        {
            ...
            {
                conectar.Abrir();
                SqlCommand command = new SqlCommand("Delete From retiro Where Id = '" + txt_id.Text + "'", conectar.SqlConnection);
                command.ExecuteNonQuery();
